...
export function ChatApp() {
  ...
  async function sendToAgent(userText: string, imageDataUrl?: string) {
    ...
    try {
      const res = await fetch("/api/inspecao-sst-ia/chat", {
        method: "POST",
  ...
  function handleMenuSelection(num: string) {
    const item = FULL_MENU.find((entry) => entry.startsWith(`${num}.`));
  ...
  const visibleMenuItems = FULL_MENU.slice(start, start + ITEMS_PER_PAGE);
          ...
          {showMenu && (
            <AssistantBubble>
              <p className="mb-2 border-b border-black/10 pb-2 text-base font-bold text-brand-green-900">
                Bem-vindo ao seu assistente inteligente para inspeções de Segurança e Saúde no
                Trabalho.
              </p>
              <p className="mb-3 text-sm text-black/70">O que deseja realizar?</p>
              ...
              <div className="mt-3 flex gap-2">
                ...
                {start + ITEMS_PER_PAGE < FULL_MENU.length && (
                  <button
                    onClick={() => setMenuPage((p) => p + 1)}
